...
# Useful Tips
  ...
  * [Configurations for Optimizing Data Loading performance for Massive Data](#configuration-for-optimizing-data-loading-performance-for-massive-data)
  * [Learnings from POC execution](#configurations-for-optimizing-carbondata-performance)
...
| counter_100 | Decimal       | NA          | Measure     |
  ...
  Add this as the first column in SORT_COLUMNS.Since the cardinality of msisdn is high (30 million),
  it is not required to create a dictionary on this column.
  ...
  For example, if MSISDN, HOST and Dime_1 are frequently-used columns, then the sort column order of table is suggested as
  Dime_1>HOST>MSISDN, because Dime_1 has the lowest cardinality.Since Dime_1 has low cardinality, Dictionary can be created for it.
  Since HOST and MSISDN have cardinalities exceeding 1,000,000, it is not required to generate dictionary on them.
  The create table command can be modified as suggested below :
  ...
  create table carbondata_table(
      ...
      TBLPROPERTIES ('SORT_COLUMNS'='Dime_1, HOST, MSISDN','DICTIONARY_INCLUDE'='Dime_1)
  ```
  
  If the frequency of each column used for filtering is similar, Place the columns in sort_columns in ascending
  order of cardinalities.It is not required to create dictionary on columns if the cardinalities are high.
...
```
  create table carbondata_table(
      ...
      TBLPROPERTIES ('SORT_COLUMNS'='Dime_2,Dime_3,Dime_1,BEGIN_TIME,HOST,MSISDN','DICTIONARY_INCLUDE'='Dime_1..')
  ```
...
## Configuration for Optimizing Data Loading performance for Massive Data
  ...
  If memory that can be configured is less, then it is preferable to slow the speed of data loading instead of data load failure.
  The following configurations can be tuned in carbon.properties file to get a better performance.
...
|carbon.merge.sort.prefetch|Default: true | You may want set this value to false if you have not enough memory|
  ...
  For example, if there are 10 million records to be loaded to CarbonData table, and the system/cluster has only 16 cores, 64GB memory,
  using the default configuration will cause data loading to fail always in sort step. Modify carbon.properties as suggested below:
...
## Configurations for Optimizing CarbonData Performance
  ...
  scenarios. After the completion of POC,the configurations tuned to improve performance have been tabulated below :
